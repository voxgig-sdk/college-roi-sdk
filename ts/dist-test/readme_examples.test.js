...
const TEST_SEED = { "entity": { "best_value": { "test01": { "id": "test01" } }, "college": { "test01": { "id": "test01" } }, "index": { "test01": { "id": "test01" } }, "major": { "test01": { "id": "test01" } }, "openapi": { "test01": { "id": "test01" } }, "out_of_state_penalty": { "test01": { "id": "test01" } }, "slug": { "test01": { "id": "test01" } }, "state": { "test01": { "id": "test01" } }, "top_50": { "test01": { "id": "test01" } }, "worst_roi_major": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CollegeRoiSDK\b/.test(code) ||
        /\bCollegeRoiSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CollegeRoiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CollegeRoiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
