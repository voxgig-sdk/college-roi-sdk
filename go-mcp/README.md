...
[MCP](https://modelcontextprotocol.io) server exposing the CollegeRoi SDK as
two agent tools — `college-roi_list` and `college-roi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/college-roi-mcp)
make build

# 2. Provide credentials via the environment
export COLLEGE_ROI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user college-roi \
  -- /absolute/path/to/college-roi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./college-roi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// college-roi_list: first page of records
{ "entity": "best_value" }
{ "entity": "best_value", "query": { } }

// college-roi_load: one record by id
{ "entity": "index", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/college-roi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user college-roi \
     -- "$PWD"/dist/*/college-roi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `college-roi_list` and `college-roi_load` tools now appear
   in new sessions. Ask the agent to *"list best_value using college-roi"*
   and it calls `college-roi_list` with `{"entity":"best_value"}`.
...
```sh
export COLLEGE_ROI_APIKEY=sk_live_xxx            # API key
export COLLEGE_ROI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `college-roi_list` tool
...
### Call the `college-roi_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `college-roi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `college-roi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COLLEGE_ROI_APIKEY` | API key sent with every request. |
| `COLLEGE_ROI_BASE` | Optional override of the API base URL. |

### Entities

The 10 entities valid as the `entity` argument:

best_value | college | index | major | openapi | out_of_state_penalty | slug | state | top_50 | worst_roi_major

### Smoke test via HTTP (raw JSON-RPC)

```sh
./college-roi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"college-roi_load","arguments":{"entity":"index","query":{"id":1}}}}'
```
