...
class StateEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->State(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->State(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set COLLEGE_ROI_TEST_STATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function state_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["state01", "state02", "state03", "best_value01", "best_value02", "best_value03"] as $k) {
    ...
    $entid_env_raw = getenv("COLLEGE_ROI_TEST_STATE_ENTID");
