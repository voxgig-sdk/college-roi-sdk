...
class StateDirectTest extends TestCase
{
    public function test_direct_list_state(): void
    {
        $setup = state_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-state", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["state01"] as $_liveKey) {
...
function state_direct_setup($mockres)
{
