...
class OpenapiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new OpenapiEntity($this->_client, $opts);
    }

    /**
     * @param Openapi|array $args Openapi data (assoc-array) to store.
     */
    ...
    /**
     * @return Openapi|array The current Openapi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Openapi fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Openapi fields).
     */
    ...
    /**
     * Load a single Openapi.
     *
     * @param OpenapiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed OpenapiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Openapi|array The loaded Openapi as an assoc-array at the
     *   SDK boundary; throws CollegeRoiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CollegeRoiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
