...
// CollegeRoi SDK OutOfStatePenalty entity
...
class OutOfStatePenaltyEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "out_of_state_penalty";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new OutOfStatePenaltyEntity($this->_client, $opts);
    }

    /**
     * @param OutOfStatePenalty|array $args OutOfStatePenalty data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CollegeRoiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return OutOfStatePenalty|array The current OutOfStatePenalty data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of OutOfStatePenalty fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CollegeRoiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of OutOfStatePenalty fields).
     */
    ...
    /**
     * List OutOfStatePenalty items matching the given filter.
     *
     * @param OutOfStatePenaltyListMatch|array|null $reqmatch Match filter (any subset
     *   of OutOfStatePenalty fields) as an assoc-array; OutOfStatePenaltyListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return OutOfStatePenalty[]|array A list of OutOfStatePenalty items as assoc-arrays at
     *   the SDK boundary; throws CollegeRoiError on failure (item-5 convention).
     */
