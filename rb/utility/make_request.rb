...
module CollegeRoiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CollegeRoiResponse.new({})
    ...
    if fetch_err
      ...
      response = CollegeRoiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CollegeRoiResponse.new(fetched)
    else
