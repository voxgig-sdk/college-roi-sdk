...
class OutOfStatePenaltyEntity
  ...
  def make
    opts = @_entopts.dup
    OutOfStatePenaltyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CollegeRoiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [OutOfStatePenalty, Hash] the current OutOfStatePenalty data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CollegeRoiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of OutOfStatePenalty fields)
  def match_get
  ...
  #
  # @param reqmatch [OutOfStatePenaltyListMatch, Hash, nil] match filter (any subset of
  #   OutOfStatePenalty fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<OutOfStatePenalty>, Array] the matching OutOfStatePenalty items; raises CollegeRoiError on failure
