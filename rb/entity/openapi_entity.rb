...
class OpenapiEntity
  ...
  def make
    opts = @_entopts.dup
    OpenapiEntity.new(@_client, opts)
  end
  ...
  # @return [Openapi, Hash] the current Openapi data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Openapi fields)
  def match_get
  ...
  #
  # @param reqmatch [OpenapiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Openapi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Openapi, Hash] the loaded Openapi; raises CollegeRoiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CollegeRoiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
