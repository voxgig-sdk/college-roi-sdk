...
class MajorDirectTest < Minitest::Test
  def test_direct_list_major
    setup = major_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-major", setup[:live] ? "live" : "unit")
    if _should_skip
...
def major_direct_setup(mockres)
