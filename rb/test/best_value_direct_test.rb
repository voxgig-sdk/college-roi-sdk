...
class BestValueDirectTest < Minitest::Test
  def test_direct_list_best_value
    setup = best_value_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-best_value", setup[:live] ? "live" : "unit")
    if _should_skip
...
def best_value_direct_setup(mockres)
