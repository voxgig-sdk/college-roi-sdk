...
func TestIndexEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		indexRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.index")))
...
func indexBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COLLEGE_ROI_TEST_INDEX_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COLLEGE_ROI_TEST_INDEX_ENTID"])
