...
import (
	...
	omni "github.com/voxgig-sdk/college-roi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CollegeRoiError); is {
				out := map[string]any{
					"name":    "CollegeRoiError",
...
func omniCtx(arg any, client *sdk.CollegeRoiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CollegeRoiSDK); is {
			client = live
