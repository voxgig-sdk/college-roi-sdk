...
class TestMajorDirect:

    def test_should_direct_list_major(self):
        setup = _major_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-major", "live" if setup["live"] else "unit")
        if _skip:
...
def _major_direct_setup(mockres):
