    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COLLEGE_ROI_TEST_INDEX_ENTID JSON to run live")
        client = setup["client"]
        ...
        index_ref01_data_dt0_loaded = index_ref01_ent.load(index_ref01_match_dt0, None)
        assert index_ref01_data_dt0_loaded is not None
...
def _index_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COLLEGE_ROI_TEST_INDEX_ENTID"))
