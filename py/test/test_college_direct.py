...
class TestCollegeDirect:

    def test_should_direct_list_college(self):
        setup = _college_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-college", "live" if setup["live"] else "unit")
        if _skip:
...
def _college_direct_setup(mockres):
