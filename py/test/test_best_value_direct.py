...
class TestBestValueDirect:

    def test_should_direct_list_best_value(self):
        setup = _best_value_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-best_value", "live" if setup["live"] else "unit")
        if _skip:
...
def _best_value_direct_setup(mockres):
