...
class TestIndexDirect:
    ...
    def test_should_direct_load_index(self):
        setup = _index_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-index", "live" if setup["live"] else "unit")
        if _skip:
