...
class CollegeRoiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CollegeRoiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CollegeRoiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CollegeRoiTestRunner.getenv("COLLEGE_ROI_TEST_LIVE")
        override = CollegeRoiTestRunner.getenv("COLLEGE_ROI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CollegeRoiTestRunner.getenv(key)
        ...
        explain = CollegeRoiTestRunner.getenv("COLLEGE_ROI_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CollegeRoiTestRunner._test_control is not None:
            return CollegeRoiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CollegeRoiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CollegeRoiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CollegeRoiTestRunner.load_test_control()
...
def load_env_local():
    CollegeRoiTestRunner.load_env_local()


def env_override(m):
    return CollegeRoiTestRunner.env_override(m)


def entity_data(v):
    return CollegeRoiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CollegeRoiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CollegeRoiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CollegeRoiTestRunner.load_test_control()


def live_client_options():
    return CollegeRoiTestRunner.live_client_options()


def live_delay_ms():
    return CollegeRoiTestRunner.live_delay_ms()
