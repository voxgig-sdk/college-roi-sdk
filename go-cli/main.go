// Command CollegeRoi-cli is a CLI + REPL for the CollegeRoi SDK,
...
const prompt = "college-roi"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "best_value college index major openapi out_of_state_penalty slug state top_50 worst_roi_major"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COLLEGE_ROI_APIKEY (api key), COLLEGE_ROI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COLLEGE_ROI_APIKEY carries the API key and
	// COLLEGE_ROI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COLLEGE_ROI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COLLEGE_ROI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCollegeRoiSDK(opts)
