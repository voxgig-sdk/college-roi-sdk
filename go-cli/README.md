...
boru-driven command-line client **and** interactive REPL for the CollegeRoi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/college-roi-cli)
make build

# 2. See usage (words, entities, env vars)
./college-roi-cli --help

# 3. Provide credentials once, via the environment
export COLLEGE_ROI_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./college-roi-cli list best_value
./college-roi-cli list college

# 5. Override the API base URL for a single call
COLLEGE_ROI_BASE=https://api.example.com ./college-roi-cli list best_value

# 6. No arguments -> interactive REPL
./college-roi-cli
college-roi> list best_value
college-roi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/college-roi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/college-roi-cli list best_value
   ```
...
Run with no arguments to open a REPL (prompt `college-roi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `best_value`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/college-roi-cli`. |
...
(`./college-roi-cli <expr>`) and interactively (the REPL), and expressions compose the
