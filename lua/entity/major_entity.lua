...
local MajorEntity = {}
MajorEntity.__index = MajorEntity


function MajorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MajorEntity)
  self._name = "major"
  self._client = client
...
function MajorEntity:get_name()
  return self._name
end


function MajorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MajorEntity.new(self._client, opts)
end
...
function MajorEntity:mark_deleted()
  self._deleted = true
end


function MajorEntity:deleted()
  return true == self._deleted
end


function MajorEntity:data_set(args)
  if args ~= nil then
...
function MajorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MajorEntity:match_set(args)
  if args ~= nil then
...
function MajorEntity:match_get()
...
function MajorEntity:stream(action, args, callopts)
...
---@param reqmatch MajorListMatch
---@param ctrl? table
---@return Major[]
---@return string? err
function MajorEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MajorEntity:_run_op(ctx, post_done)
  local utility = self._utility
