...
local Top50Entity = {}
Top50Entity.__index = Top50Entity


function Top50Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Top50Entity)
  self._name = "top_50"
  self._client = client
...
function Top50Entity:get_name()
  return self._name
end


function Top50Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Top50Entity.new(self._client, opts)
end
...
function Top50Entity:mark_deleted()
  self._deleted = true
end


function Top50Entity:deleted()
  return true == self._deleted
end


function Top50Entity:data_set(args)
  if args ~= nil then
...
function Top50Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Top50Entity:match_set(args)
  if args ~= nil then
...
function Top50Entity:match_get()
...
function Top50Entity:stream(action, args, callopts)
...
---@param reqmatch Top50ListMatch
---@param ctrl? table
---@return Top50[]
---@return string? err
function Top50Entity:list(reqmatch, ctrl)
  local utility = self._utility
...
function Top50Entity:_run_op(ctx, post_done)
  local utility = self._utility
