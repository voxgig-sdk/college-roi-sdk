...
local SlugEntity = {}
SlugEntity.__index = SlugEntity


function SlugEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SlugEntity)
  self._name = "slug"
  self._client = client
...
function SlugEntity:get_name()
  return self._name
end


function SlugEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SlugEntity.new(self._client, opts)
end
...
function SlugEntity:mark_deleted()
  self._deleted = true
end


function SlugEntity:deleted()
  return true == self._deleted
end


function SlugEntity:data_set(args)
  if args ~= nil then
...
function SlugEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SlugEntity:match_set(args)
  if args ~= nil then
...
function SlugEntity:stream(action, args, callopts)
...
---@param reqmatch SlugLoadMatch
---@param ctrl? table
---@return Slug
---@return string? err
function SlugEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SlugEntity:_run_op(ctx, post_done)
  local utility = self._utility
