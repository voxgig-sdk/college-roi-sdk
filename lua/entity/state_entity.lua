...
function StateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StateEntity)
  self._name = "state"
  self._client = client
...
function StateEntity:get_name()
  return self._name
end


function StateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StateEntity.new(self._client, opts)
end
...
function StateEntity:mark_deleted()
  self._deleted = true
end


function StateEntity:deleted()
  return true == self._deleted
end


function StateEntity:data_set(args)
  if args ~= nil then
...
function StateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StateEntity:match_set(args)
  if args ~= nil then
...
function StateEntity:match_get()
...
function StateEntity:stream(action, args, callopts)
...
function StateEntity:_run_op(ctx, post_done)
  local utility = self._utility
