...
local BestValueEntity = {}
BestValueEntity.__index = BestValueEntity


function BestValueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BestValueEntity)
  self._name = "best_value"
  self._client = client
...
function BestValueEntity:get_name()
  return self._name
end


function BestValueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BestValueEntity.new(self._client, opts)
end
...
function BestValueEntity:mark_deleted()
  self._deleted = true
end


function BestValueEntity:deleted()
  return true == self._deleted
end


function BestValueEntity:data_set(args)
  if args ~= nil then
...
function BestValueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BestValueEntity:match_set(args)
  if args ~= nil then
...
function BestValueEntity:match_get()
...
function BestValueEntity:stream(action, args, callopts)
...
---@param reqmatch BestValueListMatch
---@param ctrl? table
---@return BestValue[]
---@return string? err
function BestValueEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BestValueEntity:_run_op(ctx, post_done)
  local utility = self._utility
