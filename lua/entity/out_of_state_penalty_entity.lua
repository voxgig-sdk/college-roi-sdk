...
local OutOfStatePenaltyEntity = {}
OutOfStatePenaltyEntity.__index = OutOfStatePenaltyEntity


function OutOfStatePenaltyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OutOfStatePenaltyEntity)
  self._name = "out_of_state_penalty"
  self._client = client
...
function OutOfStatePenaltyEntity:get_name()
  return self._name
end


function OutOfStatePenaltyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OutOfStatePenaltyEntity.new(self._client, opts)
end
...
function OutOfStatePenaltyEntity:mark_deleted()
  self._deleted = true
end


function OutOfStatePenaltyEntity:deleted()
  return true == self._deleted
end


function OutOfStatePenaltyEntity:data_set(args)
  if args ~= nil then
...
function OutOfStatePenaltyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OutOfStatePenaltyEntity:match_set(args)
  if args ~= nil then
...
function OutOfStatePenaltyEntity:match_get()
...
function OutOfStatePenaltyEntity:stream(action, args, callopts)
...
---@param reqmatch OutOfStatePenaltyListMatch
---@param ctrl? table
---@return OutOfStatePenalty[]
---@return string? err
function OutOfStatePenaltyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function OutOfStatePenaltyEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return OutOfStatePenaltyEntity
