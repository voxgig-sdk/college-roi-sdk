...
local CollegeEntity = {}
CollegeEntity.__index = CollegeEntity


function CollegeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollegeEntity)
  self._name = "college"
  self._client = client
...
function CollegeEntity:get_name()
  return self._name
end


function CollegeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollegeEntity.new(self._client, opts)
end
...
function CollegeEntity:mark_deleted()
  self._deleted = true
end


function CollegeEntity:deleted()
  return true == self._deleted
end


function CollegeEntity:data_set(args)
  if args ~= nil then
...
function CollegeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollegeEntity:match_set(args)
  if args ~= nil then
...
function CollegeEntity:match_get()
...
function CollegeEntity:stream(action, args, callopts)
...
---@param reqmatch CollegeListMatch
---@param ctrl? table
---@return College[]
---@return string? err
function CollegeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CollegeEntity:_run_op(ctx, post_done)
  local utility = self._utility
