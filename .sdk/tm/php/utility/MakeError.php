...
// CollegeRoi SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class CollegeRoiMakeError
{
    public static function call(?CollegeRoiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CollegeRoiContext([], null);
        }
        $op = $ctx->op ?? new CollegeRoiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CollegeRoiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CollegeRoiError) ? $err->msg : (string)$err;
        $msg = "CollegeRoiSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new CollegeRoiError('', $msg, $ctx);
        ...
        if ($err instanceof CollegeRoiError) {
